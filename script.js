...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CONSTS = {
        ...
        CATEGORIA_DESPESA: { ORDINARIA: 'ordinaria', CARTAO_CREDITO: 'cartao_credito' },
        FREQUENCIA: { UNICA: 'unica', RECORRENTE: 'recorrente', PARCELADA: 'parcelada' },
        CADASTRO_PARCELA: { VALOR_TOTAL: 'valor_total', VALOR_PARCELA: 'valor_parcela' },
        ACAO_SERIE: { EXCLUIR: 'excluir', EDITAR: 'editar' },
        TIPO_RENDERIZACAO: { RECEITA: 'receita_individual', DESPESA: 'despesa_ordinaria', FATURA: 'fatura_cartao', ORCAMENTO: 'orcamento' },
    ...
    function traduzirErroAuth(errorCode) {
        switch (errorCode) {
            ...
            case 'auth/wrong-password': return 'Senha inválida. Tente novamente.';
    ...
    function carregarDadosDoFirestore() {
        ...
        if (!currentUser) return;
        console.log("Carregando dados iniciais do Firestore para o usuário:", currentUser.uid);
        ...
        try {
            ...
            transacoes = transacoesSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            cartoes = cartoesSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            orcamentos = orcamentosSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            orcamentosFechados = orcamentosFechadosSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            ajustesFatura = ajustesFaturaSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            dividasTerceiros = dividasTerceirosSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            pessoas = pessoasSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
            pessoas.sort((a, b) => a.nome.localeCompare(b.nome));
    ...
    function renderizarDividasDoMes() {
        ...
        if (dividasDoMes.length === 0) {
            listaUl.innerHTML = '<li style="text-align: center; padding: 20px; color: #777;">Nenhuma dívida para este mês.</li>';
            return;
        }
        ...
        const listaOrdenada = Object.values(dividasAgrupadas).sort((a, b) => a.nomePessoa.localeCompare(b.nomePessoa));
...
async function excluirDividaTerceiroUnica(dividaId) {
    ...
    function renderizarResultadosBusca(resultados, termoBusca) {
        ...
        resultados.forEach(item => {
            ...
            const valorFormatado = item.tipo === 'receita' ? `+ ${formatCurrency(item.valor)}` : `- ${formatCurrency(item.valor)}`;
    ...
    function abrirModalDetalhesSerie(serieId) {
        const transacoesDaSerie = transacoes.filter(t => t.serieId === serieId).sort((a, b) => a.parcelaAtual - b.parcelaAtual);
    ...
    function atualizarResumoFinanceiro() {
        ...
        receitasDoMes = transacoesDoMesReferencia.filter(t => t.tipo === CONSTS.TIPO_TRANSACAO.RECEITA).reduce((total, t) => total + t.valor, 0);

        const despesasNaoOrcadas = transacoesDoMesReferencia.filter(t => t.tipo === CONSTS.TIPO_TRANSACAO.DESPESA && !t.orcamentoId);
        despesasDoMes += despesasNaoOrcadas.reduce((total, t) => total + t.valor, 0);

        orcamentos.forEach(orcamento => {
            const gastosNesteOrcamento = transacoesDoMesReferencia.filter(t => t.orcamentoId === orcamento.id).reduce((total, t) => total + t.valor, 0);
        ...
        const totalAjustesDoMes = ajustesFatura.filter(a => a.mesAnoReferencia === mesAnoAtual).reduce((total, a) => total + a.valor, 0);
    ...
    function resetModalCartao() { 
        if (!nomeCartaoInputModal || !diaVencimentoFaturaInputModal || !cartaoEditIdInput || !modalCartaoTitulo || !btnSalvarCartaoModalBtn) return; 
        if (!isCartaoEditMode) { 
    ...
    document.querySelectorAll('.close-button, .close-button-footer').forEach(button => { 
        button.addEventListener('click', () => { 
            const modalId = button.dataset.modalId; 
            const modalParaFechar = modalId ? document.getElementById(modalId) : button.closest('.modal'); 
    ...
    function preencherModalParaEdicao(id) { 
        if (!tipoTransacaoSelect || !nomeTransacaoInput || !modalHeaderNovaTransacao) return; 
        ...
        const nomeOriginal = transacao.serieId ? transacao.nome.replace(/\s\(\d+\/\d+\)$/, '').replace(/\s\(Recorrente\)$/, '') : transacao.nome; 
        const nomeCurto = nomeOriginal.substring(0, 25) + (nomeOriginal.length > 25 ? '...' : ''); 
    ...
    if (btnAbrirModalNovaTransacao) { 
        btnAbrirModalNovaTransacao.addEventListener('click', () => abrirModalEspecifico(modalNovaTransacao, null, 'transacao')); 
    }
    ...
    if (btnVoltarTransacao) { 
        btnVoltarTransacao.addEventListener('click', () => { 
            if (!passo2Container || !tipoTransacaoSelect || !nomeTransacaoInput || !btnAvancarTransacao || !btnVoltarTransacao || !btnSalvarTransacao || !modalHeaderNovaTransacao) return; 
            passo2Container.innerHTML = ""; 
            passo2Container.style.display = 'none'; 
            tipoTransacaoSelect.parentElement.style.display = 'block'; 
            nomeTransacaoInput.parentElement.style.display = 'block'; 
    ...
    function resetFormParaNovaDespesaCartao() { 
        ...
        nomeTransacaoInput.value = ''; 
        const frequenciaSelect = passo2Container.querySelector('#frequenciaDespesaCartao'); 
        if (frequenciaSelect) { 
            frequenciaSelect.value = CONSTS.FREQUENCIA.UNICA; 
            frequenciaSelect.dispatchEvent(new Event('change')); 
        } 
        const valorUnicaRecorrenteInput = passo2Container.querySelector('#valorDespesaCartaoUnicaRecorrente'); 
        if (valorUnicaRecorrenteInput) valorUnicaRecorrenteInput.value = ''; 
        const valorParceladaInput = passo2Container.querySelector('#valorDespesaCartaoParcelada'); 
        if (valorParceladaInput) valorParceladaInput.value = ''; 
        const qtdParcelasInput = passo2Container.querySelector('#qtdParcelasCartao'); 
        if (qtdParcelasInput) qtdParcelasInput.value = ''; 
        const parcelaAtualInput = passo2Container.querySelector('#parcelaAtualCartao'); 
        if (parcelaAtualInput) parcelaAtualInput.value = '1'; 
        const orcamentoSelect = passo2Container.querySelector('#orcamentoVinculado'); 
        ...
        setTimeout(() => { quickAddFeedback.style.display = 'none'; }, 2500); 
    ...
    function preencherModalEdicaoCartao(cartaoId) { 
        ...
        if (cartao && modalCadastrarCartao) { 
            ...
            if (diaVencimentoFaturaInputModal) diaVencimentoFaturaInputModal.value = cartao.diaVencimentoFatura; 
            if (btnSalvarCartaoModalBtn) btnSalvarCartaoModalBtn.textContent = "Salvar Alterações"; 
        } 
    }
        ...
        btnSalvarOrcamento.addEventListener('click', async () => {
            ...
            try {
                if (id) {
                    ...
                    await orcamentosCollectionRef.add({ nome: nome, valor: valor, dia: dia });
                    await registrarUltimaAlteracao();
    ...
    function popularModalRelatorio(date) {
        ...
        orcamentos.forEach(orcamento => {
            const gastosNesteOrcamento = despesasDoMes.filter(t => t.orcamentoId === orcamento.id).reduce((sum, t) => sum + t.valor, 0);
    ...
    function inicializarTransacoes() { return []; }
    function inicializarCartoes() { return []; }
    function inicializarAjustes() { return []; }
    function inicializarOrcamentos() { return []; }
    function inicializarOrcamentosFechados() { return []; }
